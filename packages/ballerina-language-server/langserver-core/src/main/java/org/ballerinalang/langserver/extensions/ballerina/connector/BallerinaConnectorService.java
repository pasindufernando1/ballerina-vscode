...
public class BallerinaConnectorService implements ExtendedLanguageServerService {
    ...
    private Path resolveBalaPath(String org, String pkgName, String version) throws LSConnectorException {
        ...
        Collection<ResolutionResponse> resolutionResponses = packageResolver.resolvePackages(
                ...
                ResolutionOptions.builder().setOffline(resolvesOffline()).build());
    ...
    /**
     * Whether this server resolves without Ballerina Central. Wrapped in a method because several methods here take an
     * {@code org} parameter, which would shadow the package name in a fully qualified reference.
     *
     * @return {@code true} when resolution must stay offline.
     */
    private static boolean resolvesOffline() {
        return org.ballerinalang.langserver.common.utils.PackageResolver.get().isOffline();
    }
